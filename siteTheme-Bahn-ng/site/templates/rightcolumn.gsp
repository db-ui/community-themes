...
<% if (config.site_gitRepoUrl) { %>
        <a href="${(content?.gitrepourl?:content?.'jbake-gitrepourl')?:config.site_gitRepoUrl}/${(content?.gitfilepath?:content?.'jbake-gitfilepath')?:sourceFileName}"
           target="_blank"><i class="fa fa-edit fa-fw"></i> Improve this doc</a>
<% } %>
<% if (config.site_gitHistoryUrl) { %>
